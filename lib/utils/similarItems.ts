...
// similar items
const similerItems = (currentItem: PostItem[], allItems: PostItem[], slug: string): PostItem[] => {
  let categories: string[] = [];
  let tags: string[] = [];

  // set categories
  if (currentItem[0]?.frontmatter?.categories && currentItem[0].frontmatter.categories.length > 0) {
    categories = currentItem[0].frontmatter.categories;
  }

  // set tags
  if (currentItem[0]?.frontmatter?.tags && currentItem[0].frontmatter.tags.length > 0) {
